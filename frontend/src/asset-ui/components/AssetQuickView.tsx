...
export function AssetQuickView({
    ...
    <>
      <div className="flex items-start justify-between gap-3">
        ...
        {variant === 'drawer' && onClose ? (
          <button type="button" className="rounded-xl p-2 text-ink-faint transition hover:bg-surface-2 hover:text-ink" onClick={onClose}>
            <X className="h-4 w-4" />
          </button>
        ) : null}
      </div>
  ...
  if (variant === 'panel') {
    return <aside className="surface-card sticky top-24 animate-fade-up overflow-hidden p-5">{content}</aside>;
  }

  return (
    <div className="fixed inset-0 z-40">
      <button type="button" className="h-full w-full bg-slate-950/55" onClick={onClose} />
      <aside className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto border-l border-line bg-canvas p-5 shadow-panel">
        {content}
      </aside>
    </div>
  );
